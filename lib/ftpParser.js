    ...
    unix: function(entry) {
        ...
        if (group) {
            ...
            var name = group[21];
	          ...
	          var isTime = group[20].indexOf(':');

	          if (isTime > -1 ) {
		    // checking if the year is prompted
		            var now = new Date();
		            if (now.getMonth() < 5 && new Date(group[19]).getMonth() >= now.getMonth()) {
	          //if today is after the 1st of july we know the date must be from the sema year.
			      //if not the only way it could be from previous year is when the month is grater than current month.
			              var date = new Date(group[19] + " " + (1900 + now.getYear() - 1)).getTime();
		            } else {
			              var date = new Date(group[19] + " " + (1900 + now.getYear())).getTime();
		            }
	          } else {
