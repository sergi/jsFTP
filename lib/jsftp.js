...
(function() {
    ...
    this.append = function(filePath, buffer, callback) {
        this.orderedPasv({ cmd: "appe " + filePath }, function(err, socket) {
            if (err) return callback(err);
            ...
            setTimeout(function() {
                if (socket && socket.writable) {
                    ...
                    console.log("ftp error: couldn't retrieve connection for 'appe " + filePath + "'.");
                }
            }, 100);
        });
    };
